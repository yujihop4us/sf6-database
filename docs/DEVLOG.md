...
## 2026-08-24 — 大会終了後処理の自動化（手動運用からの脱却）

### 背景
大会終了後のデータ反映が手動運用のため、実行漏れ・失敗が誰にも気づかれず
データ欠落が蓄積していた。実際に起きていた欠落はすべて**無警告**だった。

### 実装
- `scripts/finalize-tournaments.js` を新規作成
  - start.gg を正として DB との差分を検出し、セット/順位/キャラを自動補完
  - **冪等・検証駆動・状態列なし**の設計。毎回突き合わせるので再実行で自然に治り、
    DDL 不要（`finalized_at` のような状態列を持たない）
  - `--dry-run` / `--tournament-id` / `--days` に対応
  - 「要対応(gaps)」と「参考情報(notes)」を分離。要対応時のみ exit 1 して通知する。
    毎日同じ警告が出て誰も見なくなる状態（今回の問題の本質）を避けるため
- `.github/workflows/finalize-tournaments.yml` を新規作成（日次 18:00 UTC + 手動実行）

### 実行基盤に GitHub Actions を選んだ理由
Vercel は **Hobby プラン**のため Cron は日1回・関数は60秒上限。
CEO 2026 は 2225 セットで数十分かかるため Vercel では処理できない。
リポジトリが public なので GitHub Actions は時間無制限・無料で使える。

### 併せて修正した「無警告の失敗」
- `import-sets.js`: バッチ投入失敗時の**個別リトライ**と投入後の**件数検証**を追加
  （BAM16 で1件が無警告で落ちていた。単体 upsert は成功するためバッチ固有の問題）
- `src/lib/liquipedia-backfill.ts`: generic UA + 生HTML取得で **403**。
  日次 Vercel Cron が**ずっと静かに失敗していた**。MediaWiki API 経由に変更

### 検証結果（過去大会の未発見欠落が自動修復された）
| 大会 | 修復前 → 後 |
|---|---|
| Evo 2026 | セット 3404 → **5784**（約2400件回復） |
| EWC LCQ | キャラ 0 → 48 |
| BAM 16 | セット 458 → 459 |
| CEO 2026 | 順位を意図的に破壊 → **8件を自動復元**（動作検証） |

### 未解決の課題
- **PR #1 のマージが必要**。スケジュール実行はデフォルトブランチでのみ動作する
- **A（大会登録）と B（大会中のライブ取得）は未着手**。
  - A: `tournamentConfig.ts` がコードのため新大会追加にデプロイが必要。
    配信チャンネルの選定は自動判定が難しい（CEO は11ch登録されていた）
  - B: `live-fetch-v2.js` はローカル依存のまま。Hobby では Vercel Cron で代替不可
- `backfill_ceo.js` を誤って上書きした（同名ファイルがセッション開始時から存在）。
  Git 未追跡のため元の内容は復元不可
...
## 2026-08-27 — 自動化の本番稼働 + 残データ不備の解消

### PR #1 をマージし GitHub Actions が稼働開始
- 9コミット / 34ファイルを main へマージ。CI 未設定のためマージ前に
  型チェックとビルドを手動で確認
- 両ワークフローを dry-run で実行し **success** を確認
  （Secrets 読み込み・DB接続・start.gg/Liquipedia アクセスがクラウド上で動作）

| ワークフロー | スケジュール |
|---|---|
| ライブ取得（開催中の大会） | 15分ごと |
| 大会データ確定処理 | 日次 JST 03:00 |

### 次回大会の登録
- **EVO France 2026**(id=46) — 2026-10-09〜11 / Nice, FR / **CPT 2026 Premier**
  start.gg tid=896660 eid=1602225、Liquipedia URL、配信ch(EVO_France 1〜6)を設定
- TGU 2026 は start.gg に個人戦(eid=1645737)が存在するものの
  **Liquipedia にページが無く CPT 認定を確認できない**ため登録を見送った
  （憶測で cpt_event_type を設定すると Tier 判定に影響するため）

### データ不備3件を解消
| 大会 | 原因 | 結果 |
|---|---|---|
| Blink Respawn 2026 | エントラント未取り込み | 順位315件 / 賞金4件 / pts 29件 |
| Evo 2026 | `liquipedia_url` が大会トップページを指していた（SF6ページでない） | 賞金 $100,000 / pts 29件 |
| BAM 16 | `liquipedia_url` 未設定 | 賞金 $6,754 / pts 31件 |

**副次的な発見**: BAM 16 の `cpt_event_type` が未設定だった。Liquipedia で
`circuittier=Premier` を確認して修正。この項目は CPT ポイント取得の可否に直結するため、
放置すると今後も静かに取りこぼしが続いていた。

### 結果
- **2026シーズン全11大会が ✅（要対応ゼロ）**
- `cpt_points` 88件 → **177件**（cpt2026:169 / ewc2026:8）
- CPT2026 上位: Micky 650 / Booce 620 / Punk 600 pts
  （※当サイト収載大会のみの集計。選手ページにも注記済み）

### 未解決の課題
- **A / S 昇格は保留中**（現在 B 止まり。`--max-tier=B` で制御）
- **昨年以前の大会は未処理**。Evo 2023/2024/2025 等でセット欠落が1万件超
- EVO France 2026 の **phases は暫定値**。start.gg のブラケット生成後に要更新
  （CEO 2026 でも想定と実構成が異なっていた）
